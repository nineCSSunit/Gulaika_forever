...
class PromptStates(StatesGroup):
    ...
    waiting_to_send_ready = State()      # Рекомендация готовых маршрутов
...
async def start_handler(message: Message, state: FSMContext):
    ...
    text = (
        f"Привет, {user_name}! 👋\n\n"
        f"Я бот, который помогает составить идеальную прогулку. Просто расскажи, что ты хочешь: куда пойти, что посмотреть, или какое место ищешь. Чем больше информации ты дашь сразу, тем точнее будет результат. Если чего-то не хватит, я обязательно спрошу! Ты так же можешь выбрать готовую прогулку, составленную заранее!)\n\n"
        f"✨ Что я умею?\n"
        ...
        f"- Могу посоветовать готовую прогулку.\n"
        ...
        "\n\n"
        f"Начнём? 🌿\n"
        f"Нажми на кнопочку снизу, что бы составить прогулку!"
    )

    def create_start_keyboard():
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Создать свою прогулку🌿", callback_data="get_walk")],
            [InlineKeyboardButton(text="Выбрать готовую🐾", callback_data="get_ready")]
        ])
        return keyboard
...
async def handle_walk_start(callback_query: CallbackQuery, state: FSMContext):
    ...
    elif callback_query.data == "get_ready":
        await callback_query.message.answer(
            "Давай я тебе предложу готовые прогулки и ты выберешь, какая тебе нравится!")
        await state.set_state(PromptStates.waiting_to_send_ready)
        await send_next_ready(callback_query, state)


ready = [
    {
        "name": "Историческая Москва",
        "description": "Прогулка по главным историческим местам Москвы, включая знаковые здания и площади.",
        "route": "Красная Площадь;Большой Театр;Собор Василия Блаженного;Государственный исторический музей;Гум;Театральная площадь;Кремль;Александровский сад;Музей археологии Москвы;Манежная площадь;",
        "link": "some url"
    },
    {
        "name": "Современная Москва",
        "description": "Откройте для себя современное лицо столицы с её стильными зданиями, арт-объектами и уникальными пространствами.",
        "route": "ГЭС-2;Парк Зарядье;Китайгородский проезд 2;Китая-город;Космодамианская набережная;Музей Москвы;Красный Октябрь;Центр современного искусства Винзавод;Артплей;Новая Третьяковка;",
        "link": "some url"
    },
    {
        "name": "Москва литературная",
        "description": "Маршрут, погружающий в мир литературы: дом-музеи и места, связанные с великими писателями.",
        "route": "Музей-квартира Пушкина на Арбате;Музей Маяковского;Дом Гоголя;Музей Булгакова;Патриаршие пруды;Музей Тургенева;Литературный музей;Мемориальный музей Чехова;Политехнический музей;Библиотека имени Ленина;",
        "link": "some url"
    },
    {
        "name": "Зелёная Москва",
        "description": "Прогулка через зелёные островки центра города: парки, сады и уютные скверы.",
        "route": "Александровский сад;Парк Зарядье;Сквер Девичьего поля;Сад Эрмитаж;Чистопрудный бульвар;Тверской бульвар;Парк Искусств Музеон;Нескучный сад;Парк Горького;Сквер у метро Кропоткинская;",
        "link": "some url"
    }
]
...
# Рекомендация готовых маршрутов

@dp.message(PromptStates.waiting_to_send_ready)
async def send_next_ready(entity, state: FSMContext):
    user_data = await state.get_data()
    current_index = user_data.get("ready_index", 0)


    def create_ready_keyboard():
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Берём эту ?✅", callback_data="ready_chose"),
             InlineKeyboardButton(text="Следующая⏭", callback_data="ready_next")],
            [InlineKeyboardButton(text="Заново", callback_data="ready_repeat")]
        ])
        return keyboard

    if current_index < len(ready):
        recommendation = ready[current_index]
        text = (
            f"📍 **Готовый маршрут №{current_index + 1}**:\n\n"
            f"✨ **{recommendation['name']}**\n\n"
            f"🏠 Описание: **{recommendation['description']}**\n"
            f"📌 Ссылка на готовый маршрут : {recommendation['link']}\n"
        )

        keyboard = create_ready_keyboard()

        # Отправляем сообщение с клавиатурой
        try:
            # Если entity — это CallbackQuery
            await entity.message.answer(text, reply_markup=keyboard, parse_mode="Markdown")
        except AttributeError:
            # Если entity — это Message
            await entity.answer(text, reply_markup=keyboard, parse_mode="Markdown")
        current_index += 1
        await state.update_data(ready_index=current_index)
    else:
        await entity.answer("Вы просмотрели все готовые маршрутики!")
...
@router.callback_query(
    StateFilter(PromptStates.waiting_to_send_ready),
    F.data.in_(["ready_chose", "ready_next", "ready_repeat"])
)
async def handle_ready_choice(callback_query: CallbackQuery, state: FSMContext):
    ...
    if callback_query.data == "ready_chose":
        route = ready[current_index - 1]['route']
        prompt_data = {'место': route,
                       'время': 'нет информации',
                       'конечная точка': 'нет информации',
        ...
        await callback_query.answer("Твой выбор сделан! Прогулка будет базироваться на этом маршруте!")
        await callback_query.message.answer("Твой выбор сделан! Прогулка будет базироваться на этом маршруте!")
        await request_next_info(prompt_data, callback_query.message, state)

    elif callback_query.data == "ready_next":
        await callback_query.answer("Следующий готовый маршрут!")
        await callback_query.message.answer("Следующий готовый маршрут!")
        await send_next_ready(callback_query.message, state)

    elif callback_query.data == "ready_repeat":
        await callback_query.answer("Заново? Ну давай!)")
        await callback_query.message.answer("Заново? Ну давай!)")
        await state.update_data(ready_index=0)
        await send_next_ready(callback_query.message, state)
